var org_metaworks_widget_AbstractMetaworksFile = function(objectId, className){
...
//				imageDiv.html('<img style=\"display:none;\" src=\'' + image + '\'>').attr("href","javascript:mw3.objects[" + objectId + "].download()");
				...
				if(image == null){
					if(console) console.log('org_metaworks_widget_AbstractMetaworksFile : not found image(' + object.uploadedPath + ')');
...
org_metaworks_widget_AbstractMetaworksFile.prototype.extraFilename = function(filepath){
...
org_metaworks_widget_AbstractMetaworksFile.prototype.setFilename = function(filename){
...
org_metaworks_widget_AbstractMetaworksFile.prototype.addFileChangeListener = function(listener){
	this.fileChangeListeners[this.fileChangeListeners.length] = listener;
};

org_metaworks_widget_AbstractMetaworksFile.prototype.showStatus = function(message){
	if('upload DONE.' == message){
		$("#objDiv_" + this.objectId).triggerHandler('uploaded');
	}
};

org_metaworks_widget_AbstractMetaworksFile.prototype.reset = function(){
...
org_metaworks_widget_AbstractMetaworksFile.prototype.add = function(event){
	$(mw3.getInputElement(this.objectId, 'fileTransfer')).trigger(event);
}


org_metaworks_widget_AbstractMetaworksFile.prototype.del = function(event){
	var object = mw3.objects[this.objectId];
...
org_metaworks_widget_AbstractMetaworksFile.prototype.download = function(){		
	mw3.call(this.objectId, 'download');
}
org_metaworks_widget_AbstractMetaworksFile.prototype.getImage = function(){		
	mw3.call(this.objectId, 'download');
}
